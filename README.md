...
| 項目       | 内容                                       |
| -------- | ---------------------------------------- |
| テンプレート一覧 | GitHub上の `.js` ファイルを自動取得して一覧化            |
| エディタ     | Monaco Editor組込み。構文チェック・補完・シンタックスハイライト対応 |
| サジェスト    | kintoneアプリのフィールドコード・ラベルを自動提案             |
| 編集・保存    | その場で編集 → ダウンロード可能                        |
| リフレッシュ   | 最新テンプレートを再取得（キャッシュはセッション内で保持）            |
