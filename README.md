...
> 🚀 Major Update: Toolkit v2.1.0 released
...
**v2.1.0** では、フィールド・設定・JavaScript・他アプリの関係を共通の依存関係データとして整理し、
...
| **設定の整合性チェック**（Health） | JavaScriptに存在しないフィールドコードへの参照が残っていないか |
...
詳細は [リリースノート](./RELEASE_NOTES_v2.1.0.md) を参照してください。
...
- 🆕 **設定の整合性チェック** — 存在しないフィールドコードへの参照を検出
  - 表記ゆれ（大文字小文字・全角半角・記号違い）の場合は「近い既存コード」を提示
- 🆕 計算式の最大段数を表示
- プロセスフロー図をMermaidで描画
- 📊 直近500件のステータス滞留ヒート表示
