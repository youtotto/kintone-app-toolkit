...
> 🚀 Latest: Toolkit v2.2.5 released
...
| **2.2.2** | 2026-08-30 | Relationsタブを中心に、アプリ間の依存関係をより把握しやすくする改善を行いました。|
| **2.2.3** | 2026-09-01 | **JavaScript解析の検出精度を改善**<br>・「存在しないフィールド参照」の検出精度を改善<br>・表示ラベルや選択肢値をフィールドコードとして誤検出する問題を修正<br>・動的なテンプレートリテラル参照を、存在しない静的フィールドとして扱わないよう改善<br>・JavaScriptフィールド参照解析の回帰テストを追加 |
| **2.2.4** | 2026-09-17 | **JavaScript解析の外部アプリ参照の判定を改善**<br>・REST APIで別のkintoneアプリを取得・更新するJavaScriptについて、外部アプリ側のフィールドを現在のアプリのフィールドとして誤認しないよう修正<br>・外部アプリのフィールド参照（取得／更新）を依存関係として可視化<br>・外部アプリのフィールドが「存在しない参照」として表示される問題を修正 |
| **2.2.5** | 2026-09-23 | **Relations / Fields の影響分析精度を改善**<br>・Relationsで「走査する」を実行した後、Fieldsへ再読み込みなしで結果が反映されるよう修正<br>・ルックアップの「ほかのフィールドのコピー」で参照されるコピー元フィールドも、他アプリからの参照として変更影響に反映<br>・再読み込み後もキャッシュ済みの走査結果をFieldsの初回表示から反映するよう改善<br>・Fieldsの表示を「他アプリ連携 N件」に統一し、件数の意味を明確化 |
