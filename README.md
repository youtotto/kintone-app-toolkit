...
>▶  **AIプロンプト生成について**：Templates > **Documents** でMarkdownテンプレートを開くと **「AIプロンプト」** ボタンが表示され、現在のアプリ定義（`prefetchAppData`の取得結果）を元に、**ChatGPTへ貼るだけ**の要件定義プロンプトをダウンロードします。ChatGPTなどにそのまま貼り付けて利用できます。
...
| 1.6.0     | 2025-11-06     | AIボタンの仕様を変更。ボタン押下でファイルとしてダウンロード |
